Create a single horizontal sprite strip for the Codex app digital pet `weave` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Small chibi Thinking Sylph digital pet inspired by the provided Weave character art: warm orange-gold fluffy hair, amber eyes, gentle knowing smile, tiny orange kimono-like robe or cloak with simplified circuit-like trim, compact floating sylph silhouette, small crystalline/orbit accent attached to the body only if readable. Original pet mascot, not a human portrait..
